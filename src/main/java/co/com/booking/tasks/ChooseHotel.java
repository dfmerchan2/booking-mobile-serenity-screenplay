...
import static co.com.booking.userinterfaces.HotelsListPage.*;
  ...
  public <T extends Actor> void performAs(T actor) {
    actor.attemptsTo(
        ...
        Check.whether(BTN_CLOSE_BANNER.resolveFor(actor).isPresent())
            .andIfSo(MoveMouse.to(BTN_CLOSE_BANNER), Click.on(BTN_CLOSE_BANNER)),
    ...
    actor.remember(
        DESCRIPTION_BOOKING.getKey(), LBL_DESCRIPTION_BOOKING.resolveFor(actor).getText());
    actor.remember(HOTEL_VALUE.getKey(), LBL_HOTEL_PRICE.resolveFor(actor).getText());
    actor.attemptsTo(WaitUntil.the(LBL_CHECK_IN, isPresent()));
