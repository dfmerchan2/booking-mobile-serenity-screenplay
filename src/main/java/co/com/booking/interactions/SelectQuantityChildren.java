...
import static co.com.booking.utilities.enums.ValueScroll.MEDIUM_SCROLL;
...
public class SelectQuantityChildren implements Interaction {
  ...
  public <T extends Actor> void performAs(T actor) {
    int currentNumberChildren =
        Integer.parseInt(LBL_NUMBER_OF_CHILDREN.resolveFor(actor).getText());

    while (currentNumberChildren != quantityChildren) {
      actor.attemptsTo(
          Check.whether(currentNumberChildren < quantityChildren)
              .andIfSo(
                  Click.on(BTN_INCREASE_CHILDREN),
                  ScrollVerticalSearchElement.with(
                      LBL_CHILD_AGE.of(ageChildren), MEDIUM_SCROLL.getSize()),
                  Click.on(LBL_CHILD_AGE.of(ageChildren)),
                  Click.on(BTN_OK))
              .otherwise(Click.on(BTN_DECREASE_ROOMS)));
