    ...
    while (currentNumberAdults != quantity) {
      actor.attemptsTo(
          ...
          Check.whether(currentNumberAdults < quantity)
              .andIfSo(Click.on(increaseValue))
              .otherwise(Click.on(decreaseValue)));
      currentNumberAdults = Integer.parseInt(currentItemValue.resolveFor(actor).getText());
    }
  }

  public static Performable ofAdults(
      int quantity, Target increaseValue, Target decreaseValue, Target currentItemValue) {
    return instrumented(
        SelectQuantity.class, quantity, increaseValue, decreaseValue, currentItemValue);
  }

  public static Performable ofRooms(
      int quantity, Target increaseValue, Target decreaseValue, Target currentItemValue) {
    return instrumented(
        SelectQuantity.class, quantity, increaseValue, decreaseValue, currentItemValue);
  }
